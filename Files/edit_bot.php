...
// Bot bilgilerini al
try {
    $stmt = $db->prepare("SELECT u.username, u.prefix, bs.script_content, bs.json_data FROM users u LEFT JOIN bot_scripts bs ON u.id = bs.bot_id WHERE u.id = ? AND u.is_bot = 1");
    $stmt->execute([$bot_id]);
    $bot = $stmt->fetch();
} catch (PDOException $e) {
...
// Bot scriptini ve JSON verisini güncelle
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_script'])) {
    try {
        ...
        // JSON verisinin geçerliliğini kontrol et
        ...
        $stmt = $db->prepare("INSERT INTO bot_scripts (bot_id, script_content, json_data) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE script_content = ?, json_data = ?");
        $stmt->execute([$bot_id, $script_content, $json_data, $script_content, $json_data]);
...
<body class="flex h-screen">
        ...
        <div class="max-w-4xl mx-auto">
            ...
            <?php if (isset($_GET['success']) && $_GET['success'] === 'updated'): ?>
                <div class="bg-green-500/20 text-green-300 p-3 rounded-lg mb-4 text-center">
                    Bot kodu ve verileri başarıyla güncellendi!
                </div>
            <?php endif; ?>
            <form method="POST" action="edit_bot?id=<?php echo $server_id; ?>&bot_id=<?php echo $bot_id; ?>" class="bg-gray-800/50 p-6 rounded-lg shadow-lg space-y-5">
                ...
                <div>
                    <label class="block text-sm font-medium mb-2" for="json_data">Bot JSON Verisi</label>
                    <textarea name="json_data" id="json_data" class="form-input"><?php echo htmlspecialchars($bot['json_data'], ENT_QUOTES, 'UTF-8'); ?></textarea>
                    <p class="text-xs text-gray-400 mt-1">Botunuzun veri saklama alanı (JSON formatında). db.getData() ve db.setData() ile erişilir.</p>
                </div>
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-700/50">
                    ...
                    <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Kodu ve Veriyi Kaydet</button>
                </div>
            </form>
        </div>
    </main>
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const scriptEditor = CodeMirror.fromTextArea(document.getElementById('script_content'), {
                mode: 'javascript',
                theme: 'monokai',
                lineNumbers: true,
                indentUnit: 4,
            ...
            const jsonEditor = CodeMirror.fromTextArea(document.getElementById('json_data'), {
