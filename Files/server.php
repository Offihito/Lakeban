...
<script src="https://code.jquery.com/jquery-3.6.0.min.js?v=1.0"></script>
    ...
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css?v=1.0" rel="stylesheet">
  <script src="https://unpkg.com/lucide@latest"></script>
    <script src="https://cdn.jsdelivr.net/npm/emoji-mart@latest/dist/browser.js?v=1.0"></script> <!-- Emoji Mart Kütüphanesi -->
    <script src="https://cdn.jsdelivr.net/npm/sortablejs@1.15.0/Sortable.min.js?v=1.0"></script>
     <style>
            body { 
...
.stacked-message {
    margin-top: 2px !important; /* Yığılmış mesajlar için üst boşluğu koru */
    padding-top: 2px !important; /* Üst dolguyu sıfırla */
...
#mobile-profile {
    ...
    display: flex;
    align-items: center;
    background: rgba(32, 34, 37, 0.95); /* Discord-like dark background */
...
#mobile-profile button, #mobile-profile a {
    transition: all 0.3s ease-in-out;
}

#mobile-profile button:hover, #mobile-profile a:hover {
...
#notification-list {
    max-height: 96vh; /* Daha fazla içerik için */
    overflow-y: auto;
    scrollbar-width: thin; /* Modern scrollbar */
    scrollbar-color: #60a5fa rgba(255, 255, 255, 0.1); /* Scrollbar rengi */
}

/* Özel scrollbar için Webkit tarayıcılar */
...
/* Bildirim öğeleri için varsayılan stil (örnek) */
...
#notification-list .notification-item:hover {
    background: rgba(255, 255, 255, 0.05); /* Hafif hover efekti */
}
...
#notification-panel {
    background: rgba(42, 42, 42, 0.85); /* Daha yumuşak bir şeffaflık için */
    ...
    box-shadow: 0 8px 32px rgba(0, 0, 0, 0.4); /* Daha derin gölge */
    transition: transform 0.3s ease, opacity 0.3s ease; /* Açılma/kapanma animasyonu */
    opacity: 0; /* İlk başta görünmez */
    transform: translateY(-20px); /* Hafif yukarı kayma efekti */
}

#notification-panel:not(.hidden) {
    opacity: 1;
    transform: translateY(0); /* Göründüğünde orijinal pozisyona döner */
}

#notification-panel .p-3 {
    background: linear-gradient(180deg, rgba(50, 50, 50, 0.9), rgba(30, 30, 30, 0.9)); /* Hafif degrade başlık */
    border-bottom: 1px solid rgba(100, 100, 100, 0.3); /* Daha yumuşak sınır */
...
#notification-panel h3 {
    ...
    font-size: 1.2rem;
    font-weight: 600;
    letter-spacing: 0.5px; /* Harf aralığı ile modern görünüm */
}

#mark-all-read {
    color: #60a5fa; /* Daha canlı mavi ton */
    font-size: 0.85rem;
    font-weight: 500;
    transition: color 0.2s ease, transform 0.2s ease; /* Hover animasyonu */
}
...
<body>
   ...
   <div id="server-sidebar" class="w-18 bg-gray-900 flex flex-col items-center px-2 py-3 space-y-2" style="background-color: #121212;">
    <a href="directmessages" class="w-12 h-12 rounded-full flex items-center justify-center hover:bg-indigo-500 cursor-pointer relative" style="background-color: #2A2A2A;">
    ...
    <div id="server-list" class="flex-1 overflow-y-auto custom-scrollbar flex flex-col items-center space-y-2">
        ...
        <div class="relative group server-item" data-server-id="<?php echo $server['id']; ?>" draggable="true">
            ...
            <a href="server?id=<?php echo $server['id']; ?>">
...
<div id="server-context-menu" class="hidden fixed text-white rounded-xl shadow-2xl p-2 z-50 border border-gray-600 w-40 max-h-32" style="background-color: #181818">
    <div class="server-context-item px-3 py-1 hover:bg-[#3CB371] hover:text-white rounded cursor-pointer" data-action="mark-all-read">
        <div class="flex items-center gap-1">
            <i data-lucide="check-circle" class="w-3 h-3"></i>
            <span class="text-sm">Tümünü Okundu Say</span>
        </div>
    </div>
</div>
...
<!-- User Profile Mobile -->
<div id="mobile-profile" class="p-4">
    <div class="flex items-center justify-between h-full">
        <div class="flex items-center justify-around flex-1">
            <button class="flex flex-col items-center justify-center rounded-xl text-white hover:bg-gray-700 transition-all duration-300 ease-in-out" style="padding: 12px;" onclick="gotoDM()">
                <i data-lucide="home" class="w-7 h-7"></i>
                <span class="text-xs mt-2 font-medium">Anasayfa</span>
            </button>
            <button class="flex flex-col items-center justify-center rounded-xl text-white hover:bg-gray-700 transition-all duration-300 ease-in-out" style="padding: 12px;" onclick="gotoCOMM()">
                <i data-lucide="users" class="w-7 h-7"></i>
                <span class="text-xs mt-2 font-medium">Topluluklar</span>
            </button>
            <button class="flex flex-col items-center justify-center rounded-xl text-white hover:bg-gray-700 transition-all duration-300 ease-in-out" style="padding: 12px;" onclick="gotoLAKE()">
                <i data-lucide="message-square-diff" class="w-7 h-7"></i>
                <span class="text-xs mt-2 font-medium">Lakealt</span>
            </button>
            <a href="/settings" class="flex flex-col items-center justify-center rounded-xl text-white hover:bg-gray-700 transition-all duration-300 ease-in-out" style="padding: 12px;">
                <i data-lucide="settings" class="w-7 h-7"></i>
                <span class="text-xs mt-2 font-medium">Ayarlar</span>
            </a>
            <div class="relative flex items-center justify-center">
                <div class="w-12 h-12 flex items-center justify-center bg-gray-800 rounded-full cursor-pointer hover:bg-gray-700 transition-all duration-300 ease-in-out" onclick="gotoProfile()">
                    <?php
                ...
                <div class="absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full status-dot status-<?php echo htmlspecialchars($_SESSION['status'] ?? 'online'); ?> border-2 border-gray-900"></div>
            </div>
        </div>
    </div>
</div>
...
<div id="create-poll-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
    <div class="bg-[#2f3136] rounded-xl p-6 w-full max-w-md shadow-lg modalentryanimation">
       ...
       <div class="mb-4">
    <label class="block text-gray-300 text-sm mb-2">Anket Süresi</label>
    <select id="poll-duration" class="w-full p-3 rounded-lg bg-white text-black border border-gray-300 focus:ring-2 focus:ring-[#5865f2] focus:border-[#5865f2] transition-all">
...
<div id="input-area" class="input-area" style="background-color: #161616;">
<div id="message-form" class="message-form" style="<?php echo !$has_write_permission ? 'pointer-events: none; opacity: 0.5;' : ''; ?>">
    <input type="hidden" id="server_id" value="<?php echo htmlspecialchars($server_id, ENT_QUOTES, 'UTF-8'); ?>">
    <input type="hidden" id="channel_id" value="<?php echo htmlspecialchars($channel_id, ENT_QUOTES, 'UTF-8'); ?>">
    
    <!-- Diğer elementler aynı kalacak -->
    <button id="more-button" type="button" class="text-gray-400 hover:text-white p-2 hover:bg-gray-700 rounded-full">
        <i data-lucide="plus" class="w-5 h-5"></i>
    </button>
<div id="mention-suggestions" class="absolute bottom-full left-0 w-full max-w-md bg-gray-900 border border-gray-800 rounded-lg shadow-xl max-h-60 overflow-y-auto hidden z-50 custom-scrollbar">
    <ul id="suggestion-list" class="text-gray-300 divide-y divide-gray-800"></ul>
</div>
    <textarea 
        id="message-input" 
        class="message-input flex-1 bg-[#161616] text-white p-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none overflow-y-auto" 
        placeholder="<?php echo $has_write_permission ? $translations['TypeMessage'] : 'Bu kanalda yazma izniniz yok.'; ?>" 
        maxlength="2000" 
        rows="1"
        style="min-height: 48px; max-height: 180px; height: 48px; white-space: pre-wrap; font-size: 1rem;"
        <?php echo !$has_write_permission ? 'disabled' : ''; ?>
    ></textarea>

    <input type="file" id="file-input" class="file-input" <?php echo !$has_write_permission ? 'disabled' : ''; ?>>
    <label for="file-input" class="file-label">
        <i data-lucide="paperclip" class="icon"></i>
    </label>
    
    <button type="button" id="send-button" class="send-button" <?php echo !$has_write_permission ? 'disabled' : ''; ?>>
        <i data-lucide="send" class="w-5 h-5"></i>
    </button>
</div>
 <div id="more-menu" class="hidden absolute z-10 border border-gray-700 rounded-lg shadow-lg" style="background-color: #1f1f1f; bottom: 100%; margin-bottom: 8px;">
       <button id="emoji-button" class="flex items-center w-full text-gray-200 hover:bg-gray-700 p-2">
            <i data-lucide="smile" class="w-5 h-5 mr-2"></i> Emoji Ekle
        </button>
        <button id="gif-button" class="flex items-center w-full text-gray-200 hover:bg-gray-700 p-2">
            <i data-lucide="image" class="w-5 h-5 mr-2"></i> GIF Ekle
        </button>
        <button id="poll-button" class="flex items-center w-full text-gray-200 hover:bg-gray-700 p-2">
            <i data-lucide="bar-chart-2" class="w-5 h-5 mr-2"></i> Anket Oluştur
        </button>
      </div>
</div>
</div>
...
let ws = new WebSocket('wss://lakeban.com:8000');
...
function handleReactionClick(messageId, emoji) {
    ...
    .then(data => {
        if (data.success) {
            ...
            const reactionContainer = messageDiv.querySelector('.flex.flex-wrap.gap-2');
...
// Helper function to format timestamps into relative time
function formatDate(timestamp) {
    const now = new Date(); // Current date: September 04, 2025, 21:20 +03
    ...
    if (diffInSeconds < 60) {
        ...
        return `${diffInMinutes} dk önce`; // X minutes ago
    } else if (diffInHours < 24) {
        return `${diffInHours} saat önce`; // X hours ago
    } else if (diffInDays === 1) {
        return `Dün ${date.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}`; // Yesterday HH:mm
    } else {
...
function createNewMessageElement(message, previousMessageData = null) {
    ...
    messageDiv.dataset.username = message.username || 'Bilinmeyen Kullanıcı';
    ...
    if (message.avatar_url) {
        ...
        avatarDiv.className += ' bg-indigo-500 rounded-full';
...
const userInfoDiv = document.createElement('div');
    ...
    usernameSpan1.style.cursor = 'pointer';
    usernameSpan1.dataset.userId = message.sender_id;
    usernameSpan1.addEventListener('click', () => {
        openUserProfilePopup(message.sender_id);
    });
    const dateSpan = document.createElement('span');
    dateSpan.className = 'text-gray-400 text-xs timestamp timestamp-tooltip';
    dateSpan.dataset.timestamp = message.created_at_unix;
    const timestamp = parseInt(message.created_at_unix);
    if (!isNaN(timestamp)) {
        dateSpan.textContent = formatDate(timestamp); // Use relative time
        dateSpan.dataset.fullDate = new Date(timestamp * 1000).toLocaleString('tr-TR'); // Full date for tooltip
    } else {
        dateSpan.textContent = 'Şimdi';
        dateSpan.dataset.fullDate = new Date().toLocaleString('tr-TR');
    }
    userInfoDiv.appendChild(usernameSpan1);
    // Role ikonunu sadece ikon varsa ekle
    if (message.role_icon) {
        const roleIconSpan = document.createElement('span');
        roleIconSpan.className = 'role-icon ml-1 mr-2'; // Margins only applied when icon exists
        if (message.role_icon.startsWith('/uploads/')) {
            // Özel ikon (yüklenmiş resim)
            const roleIconImg = document.createElement('img');
            roleIconImg.src = window.location.origin + message.role_icon;
            roleIconImg.alt = 'Role Icon';
            roleIconImg.className = 'w-5 h-5 object-cover rounded';
            roleIconSpan.appendChild(roleIconImg);
        } else {
            // Font Awesome ikonu
            const roleIcon = document.createElement('i');
            roleIcon.className = message.role_icon; // e.g., 'fas fa-crown'
            roleIcon.style.fontSize = '16px'; // Adjust size as needed
            roleIconSpan.appendChild(roleIcon);
        }
        userInfoDiv.appendChild(roleIconSpan);
    }
    userInfoDiv.appendChild(dateSpan);
    headerDiv.appendChild(userInfoDiv);
    contentDiv.appendChild(headerDiv);
    // Üç nokta butonu ve diğer mevcut kodlar (değişmeden kalır)
    ...
    replyLi.dataset.username = message.username || 'Bilinmeyen Kullanıcı';
   ...
   if (isPoll) {
    ...
    pollContainer.className = 'poll-container bg-[#2f3136] p-4 rounded-lg mt-2 relative';

    // Bitiş zamanı
    if (pollData.end_time) {
        const endTime = new Date(pollData.end_time);
        const timer = document.createElement('div');
        timer.className = 'text-xs text-gray-400 mb-2';
        timer.id = `poll-timer-${message.id}`;
        pollContainer.appendChild(timer);

        function updateTimer() {
            const now = new Date();
            const diff = endTime - now;
            if (diff <= 0) {
                timer.textContent = 'Anket kapandı';
                timer.className = 'text-red-400';
                pollContainer.querySelectorAll('.poll-option').forEach(opt => {
                    opt.classList.remove('cursor-pointer', 'hover:bg-[#36393f]');
                    opt.onclick = null;
                });
            } else {
                ...
                timer.textContent = `${days ? days + 'g ' : ''}${hours ? hours + 'sa ' : ''}${minutes}m ${seconds}s kaldı`;
            }
        }
        updateTimer();
    ...
    pollData.options.forEach((option, index) => {
        ...
        pollOption.className = 'poll-option flex flex-col p-2 rounded hover:bg-[#36393f] cursor-pointer mb-2';
        ...
        pollOption.innerHTML = `
            <div class="flex justify-between items-center">
                ...
                <span class="text-gray-400 text-sm">${voteCount} oy (${percentage.toFixed(0)}%)</span>
            </div>
            <div class="mt-1 w-full bg-[#202225] rounded-full h-2 relative overflow-hidden">
                <div class="absolute top-0 left-0 h-full bg-[#5865f2] transition-all duration-500" 
    ...
    // Kimler oy verdi butonu
    if (totalVotes > 0) {
        const votersBtn = document.createElement('button');
        votersBtn.className = 'mt-3 text-xs text-[#5865f2] hover:underline';
        votersBtn.textContent = `${totalVotes} kişi oy verdi →`;
        votersBtn.onclick = (e) => {
            e.stopPropagation();
            showPollVoters(message.id);
        };
        pollContainer.appendChild(votersBtn);
    }
...
if (!window.chatStylesAdded) {
    ...
    style.innerHTML = `
        ...
        .reaction-container:empty {
            display: none; /* Ensure no space is taken when empty */
            margin: 0;
            padding: 0;
            height: 0;
        }
        .reaction-container:not(:empty) {
            margin-top: 8px; /* Apply margin only when reactions exist */
        }
        .stacked-message {
            margin-top: 8px; /* Increase spacing between stacked messages */
        }
    `;
    ...
    reactionContainer.className = 'flex flex-wrap gap-2';
  ...
  fetch(`get_reactions.php?message_id=${message.id}`)
    ...
    .then(reactions => {
        if (reactions.length > 0) {
            // Only append reactionContainer if there are reactions
            contentDiv.appendChild(reactionContainer);
            reactions.forEach(reaction => {
                const reactionDiv = document.createElement('div');
                reactionDiv.className = 'flex items-center bg-gray-700 rounded-full px-2 py-1 cursor-pointer hover:bg-gray-600';
    ...
    .catch(error => {
        console.error('Reaksiyon yükleme hatası:', error);
        // Do not append reactionContainer on error
    });
    lucide.createIcons();
    return messageDiv;
} 
...
function showPollVoters(messageId) {
    // Önceki modal varsa kaldır
    const existing = document.getElementById('poll-voters-panel');
    if (existing) existing.remove();

    const modal = document.createElement('div');
    modal.id = 'poll-voters-panel';
    modal.className = 'fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center modalentryanimation';
    modal.innerHTML = `
        <div class="w-full max-w-3xl rounded-lg shadow-2xl flex flex-col max-h-[80vh]" style="background-color: #202225;">
            <!-- Başlık -->
            <div class="flex items-center justify-between p-4 border-b border-gray-800">
                <h2 class="text-white text-lg font-medium flex items-center">
                    <i data-lucide="check-square" class="w-5 h-5 mr-2"></i> Oy Verenler
                    <span id="voters-count" class="ml-2 bg-indigo-500 text-white text-xs rounded-full px-2 py-1">0</span>
                </h2>
                <button id="close-voters-panel-btn" class="text-gray-400 hover:text-white transition-colors">
                    <i data-lucide="x" class="w-6 h-6"></i>
                </button>
            </div>

            <!-- İçerik Alanı -->
            <div id="voters-container" class="p-4 overflow-y-auto custom-scrollbar flex-1" style="max-height: 60vh;">
                <div class="text-center text-gray-500 mt-8 animate-pulse">
                    <div class="inline-block">
                        <div class="h-12 w-12 bg-gray-700 rounded-full mb-3"></div>
    ...
    // Kapatma olayları
    const closeBtn = modal.querySelector('#close-voters-panel-btn');
    ...
    formData.append('action', 'get_poll_voters');
    ...
    .then(data => {
        const container = modal.querySelector('#voters-container');
        const countSpan = modal.querySelector('#voters-count');

        if (data.success && data.voters.length > 0) {
            countSpan.textContent = data.voters.length;

            container.innerHTML = data.voters.map((v, i) => `
                <div class="flex items-center gap-3 p-3 rounded-lg hover:bg-[#2a2d31] transition-all duration-200 cursor-default group">
                    <div class="relative flex-shrink-0">
                        <img src="${v.avatar_url || '/default-avatar.png'}" 
                             class="w-10 h-10 rounded-full ring-2 ring-[#2f3136] object-cover">
                        ${v.is_online ? '<div class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-[#202225]"></div>' : ''}
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="text-white font-medium text-sm truncate" style="color: ${v.role_color || '#ffffff'}">
                            ${escapeHtml(v.username)}
                        </div>
                        <div class="text-xs text-gray-400 flex items-center gap-2 mt-1">
                            <span class="px-2 py-0.5 bg-[#1e1f22] rounded text-[10px] font-semibold text-white">
                                Seçenek ${parseInt(v.option_index) + 1}
                            </span>
                            <span class="text-gray-500">•</span>
                            <span>${new Date(v.created_at).toLocaleString('tr-TR', {
                                hour: '2-digit',
                                minute: '2-digit',
                                day: 'numeric',
                                month: 'short'
            ...
            container.innerHTML = `
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-400 text-sm font-medium">Henüz kimse oy kullanmadı.</p>
                    <p class="text-xs text-gray-500 mt-1">İlk oyu sen ver!</p>
                </div>
            `;
        }
    })
    .catch(err => {
        ...
        container.innerHTML = `
            <div class="text-center py-10">
                <p class="text-red-400 text-sm font-medium">⚠️ Yüklenemedi</p>
                <p class="text-xs text-gray-500 mt-1">Bir hata oluştu, lütfen tekrar deneyin.</p>
            </div>
        `;
    });
}
...
ws.send(JSON.stringify({
            type: 'message-sent',
            serverId: serverId,
            channelId: currentChannelId1,
            senderId: currentUserId1,
            message: data.message,  // Tam mesaj objesi (PHP'den dönen)
...
async function sendMessage() {
    ...
    try {
        ...
        if (data.success) {
            ...
            /* ==============================================================
               1. KULLANICI MESAJI → DOM + WebSocket
               ============================================================== */
            if (data.message) {
                if (!data.message.created_at_unix) {
                    data.message.created_at_unix = Math.floor(Date.now() / 1000);
                ...
                const userElement = createNewMessageElement(data.message, previousMessageData);
                processEmojisInMessage(userElement);
                messageContainer.appendChild(userElement);

                // WebSocket'e kullanıcı mesajını gönder
                ws.send(JSON.stringify({
                    type: 'message-sent',
                    serverId: serverId,
                    channelId: currentChannelId1,
            ...
            /* ==============================================================
               2. BOT MESAJI → SADECE WebSocket (DOM'a EKLEME!)
               ============================================================== */
            if (data.bot_message) {
                if (!data.bot_message.created_at_unix) {
                    data.bot_message.created_at_unix = Math.floor(Date.now() / 1000);
                }
                data.bot_message.sender_id = String(data.bot_message.sender_id);

                // BOT MESAJINI DOM'A EKLEMEYİ KALDIRIYORUZ
                // SADECE WebSocket'e gönderiyoruz, böylece diğer kullanıcılar görebilir
                
                // WebSocket'e bot mesajını gönder → TÜM KULLANICILAR GÖRÜR
                ws.send(JSON.stringify({
                    type: 'message-sent',
                    serverId: serverId,
                    channelId: currentChannelId1,
                    senderId: data.bot_message.sender_id,
                    message: data.bot_message,
                    files: data.bot_message.files || [],
                    isBotResponse: true // Bot mesajı olduğunu işaretle
                }));

                console.log('[DEBUG] Bot mesajı WebSocket üzerinden gönderildi, DOM\'a eklenmedi');
            }

            // Genel scroll (son mesaj)
            messageContainer.scrollTop = messageContainer.scrollHeight;

            /* ==============================================================
               3. LAST READ GÜNCELLEME (sadece kullanıcı mesajı için)
               ============================================================== */
            if (data.message) {
                try {
...
document.getElementById('poll-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (data.success) {
            // 1. Modal'ı kapat ve formu sıfırla
            ...
            // 2. DOM'a anket mesajını ekle (sendMessage gibi)
            const messageContainer = document.getElementById('message-container');
            let previousMessageData = null;
            if (messageContainer.lastElementChild) {
                const lastMessage = messageContainer.lastElementChild;
            ...
            // Unix timestamp ekle (varsa)
            if (!data.message.created_at_unix) {
                data.message.created_at_unix = Math.floor(Date.now() / 1000);
            }
            data.message.sender_id = String(data.message.sender_id);

            // createNewMessageElement ile DOM'a ekle
            const newMessage = createNewMessageElement(data.message, previousMessageData);
            processEmojisInMessage(newMessage); // Emoji desteği
            messageContainer.appendChild(newMessage);
            ...
            // lastMessageId güncelle
            if (!lastMessageId || data.message.id > lastMessageId) {
                lastMessageId = data.message.id;
            }

            // 3. WebSocket ile diğer kullanıcılara gönder
            ...
            // 4. Son okunan mesajı güncelle (isteğe bağlı)
            try {
                const updateReadResponse = await fetch('update_last_read.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            ...
            showToast('error', data.message || 'Anket oluşturulamadı.');
        }
    } catch (error) {
...
// Yeni eklenen fonksiyon: Mesaj içindeki emojileri işler
function processEmojisInMessage(messageElement) {
    // Ana mesaj içeriğini işle
    const messageContent = messageElement.querySelector('.message-content');
    if (messageContent) {
        messageContent.innerHTML = replaceEmojiNamesWithImages(messageContent.innerHTML);
    }
    
    // Reply mesaj içeriğini işle (eğer varsa)
    const replyContent = messageElement.querySelector('.reply-content');
    if (replyContent) {
        replyContent.innerHTML = replaceEmojiNamesWithImages(replyContent.innerHTML);
    }
    
    // Mesaj metni içeren diğer elementleri işle
    const textElements = messageElement.querySelectorAll('[class*="text"], [class*="content"]');
...
// Mevcut replaceEmojiNamesWithImages fonksiyonunu iyileştirelim
function replaceEmojiNamesWithImages(text) {
    if (!text || !serverEmojis || !Array.isArray(serverEmojis)) {
        return text;
    }
    ...
    serverEmojis.forEach(emoji => {
        if (!emoji.emoji_name || !emoji.emoji_url) return;
        
        // Daha iyi regex: sadece :emoji_name: formatını yakala
        // ve HTML attribute'lerine müdahale etmesin
        const regex = new RegExp(`:${emoji.emoji_name}:`, 'g');
        processedText = processedText.replace(regex, `<img src="${emoji.emoji_url}" alt="${emoji.emoji_name}" class="emoji-img inline-block w-6 h-6 align-middle">`);
    });
   ...
   sendButton.addEventListener('click', async function(event) {
    ...
    event.stopPropagation(); // Ek olarak event yayılımını durdur
    await sendMessage();
});
...
// Enter tuşu ile gönderme kontrolü - GÜNCELLENMİŞ
messageInput.addEventListener('keydown', async (e) => {
    if (e.key === 'Enter' && !e.shiftKey) {
        e.preventDefault();
        e.stopPropagation(); // Ek olarak event yayılımını durdur
        
        // Formun submit olayını manuel olarak engelle
        if (e.target.form) {
            e.target.form.onsubmit = function(event) {
                event.preventDefault();
                return false;
            };
        }
...
// Formun varsayılan submit davranışını tamamen engelle
messageForm.onsubmit = function(event) {
    event.preventDefault();
    return false;
};

// Formun submit metodunu override et (ek güvenlik)
messageForm.submit = function() {
    console.log('Form submit attempt blocked');
    return false;
};
});
...
if (window.innerWidth <= 768) {
  ...
  let isHorizontalSwipe = null; // null: henüz belli değil, true: yatay, false: dikey
  let dragOffset = 0;
  ...
  function onTouchStart(e) {
    ...
    isDragging = true;
    isHorizontalSwipe = null; // Henüz yön belirlenmedi
    dragOffset = 0;
  ...
  function onTouchMove(e) {
    ...
    if (isHorizontalSwipe === null) {
      ...
      // Eğer yatay hareket dikey hareketten fazlaysa
    ...
    // Eğer dikey kaydırma ise, hiçbir şey yapma
    if (isHorizontalSwipe === false) {
      return;
    }
    
    // Eğer yatay kaydırma ise, varsayılan scroll davranışını engelle
  ...
  function onTouchEnd() {
    ...
    // Eğer dikey kaydırma idiyse, hiçbir şey yapma
    if (isHorizontalSwipe === false) {
  ...
  document.addEventListener("touchmove", onTouchMove, { passive: false }); // passive: false çünkü preventDefault kullanıyoruz
...
function reconnectWebSocket() {
    ...
    setTimeout(() => {
        ws = new WebSocket('wss://lakeban.com:8000');
        ws.onopen = () => {
            console.log('Yeniden bağlandı');
            ws.send(JSON.stringify({ type: 'auth', userId: String(currentUserId1) }));
            reconnectAttempts = 0;
        };
        ws.onerror = () => {
            reconnectAttempts++;
...
ws.onmessage = async (event) => {
  ...
  if (data.type === 'voice-channel-update') {
    if (data.channelId && data.participants) {
        try {
            updateVoiceChannelUI(data.channelId, { participants: data.participants });
            console.log(`Voice channel updated: ${data.channelId}, participants: ${data.participants.length}`);
        } catch (err) {
            console.error(`Voice channel update error for channel ${data.channelId}:`, err);
        }
    } else {
        console.warn(`Voice channel update ignored: Missing channelId or participants`, data);
    }
    return;
}

if (data.type === 'voice-user-joined') {
    if (data.channelId && data.userId && data.username && data.avatar_url) {
        try {
            updateVoiceChannelUI(data.channelId, {
                participants: [{
                    id: data.userId,
            ...
            console.log(`Voice user joined: ${data.userId} in channel ${data.channelId}`);
        } catch (err) {
            console.error(`Voice user joined error for user ${data.userId} in channel ${data.channelId}:`, err);
        }
    } else {
        console.warn(`Voice user joined ignored: Missing required fields`, data);
    }
    return;
}
...
if (data.type === 'voice-user-left') {
    if (data.channelId && data.userId) {
        try {
            ...
            console.log(`Voice user left: ${data.userId} from channel ${data.channelId}`);
        } catch (err) {
            console.error(`Voice user left error for user ${data.userId} in channel ${data.channelId}:`, err);
        }
    } else {
        console.warn(`Voice user left ignored: Missing channelId or userId`, data);
    }
    return;
}
...
let isFetching = false; // Fetch kilidi
let messageQueue = []; // Mesaj kuyruğu

// Mesajları ID'ye göre sıralama (hızlı mesajlarda created_at_unix aynı olabilir)
function sortMessagesById(messages) {
    return messages.sort((a, b) => parseInt(a.id, 10) - parseInt(b.id, 10));
}
...
function processMessageQueue() {
    ...
    if (messageQueue.length === 0) {
        console.log('[DEBUG] Kuyruk boş, işlem yapılmadı');
        return;
    }

    // Kuyruğu ID'ye göre sırala
    messageQueue = sortMessagesById(messageQueue);
    ...
    while (messageQueue.length > 0) {
        const message = messageQueue.shift(); // İlk mesajı al
        console.log('[DEBUG] İşlenen mesaj:', message);

        // created_at_unix yoksa varsayılan ata
        if (!message.created_at_unix) {
            console.warn('[DEBUG] created_at_unix eksik, varsayılan atanıyor:', message.id);
            message.created_at_unix = Math.floor(Date.now() / 1000);
        }

        // Mesajın zaten mevcut olup olmadığını kontrol et
        ...
        if (!existingMessage) {
            ...
            if (!lastMessageId || parseInt(message.id, 10) > parseInt(lastMessageId, 10)) {
                lastMessageId = message.id;
            }

            // Mesajları yığınlama kontrolü
            ...
            console.log('[DEBUG] Mesaj zaten mevcut, atlanıyor:', message.id);
        }
    }
    ...
    // Mesaj sırasını logla
    console.log('[DEBUG] DOM\'daki mesaj sırası:', Array.from(messageContainer.querySelectorAll('.message-text')).map(el => el.textContent.trim()));
}
...
function pollNewMessages() {
    ...
    if (isFetching) {
        console.log('[DEBUG] Önceki istek tamamlanmadı, yeni istek bekliyor');
        return;
    }
    ...
    .then(data => {
        if (data.success && data.messages && data.messages.length > 0) {
            console.log('[DEBUG] Yeni mesajlar alındı:', data.messages);
            // Yeni mesajları kuyruğa ekle
            messageQueue = messageQueue.concat(data.messages);
            // Kuyruğu işle
            processMessageQueue();
        } else {
            console.log('[DEBUG] Yeni mesaj yok:', data.message || 'Boş cevap');
        }
    })
    .catch(error => {
...
async function changeChannel(channelId, channelName) {
    ...
    try {
        ...
        if (messageData.success && permissionData.success) {
            ...
            updateChannelName(messageData.channel_name || channelName);
            ...
            // 9. Mobil için slayt animasyonunu tetikle
            if (window.innerWidth <= 768) {
                stage = 1; // main-content (sohbet) panelini göster
                updateSagOrtaDiv(); // Mobil görünümü güncelle
                console.log('[DEBUG] Mobile: stage set to 1, sohbet paneli açıldı');
            }
        ...
        // Hata durumunda mobil için kanal listesine dön
        if (window.innerWidth <= 768) {
            stage = 0;
            updateSagOrtaDiv();
        }
...
// Global variable for previous unread count (assuming it's defined elsewhere or add here)
...
ws.addEventListener('message', (event) => {
  try {
    ...
    if (data.type === 'unread-update') {
      // Toplam okunmamış mesaj sayısını hesapla (tüm arkadaşlar dahil)
      ...
      // currentFriendId olmadan, tüm counts'ları UI için kullan (aktif sohbet ayrımı yapılmadan)
...
// Sayfada yüklendiğinde ve periyodik olarak güncelleme (polling kaldırıldı, WS ile gerçek zamanlı güncellemeler kullanılacak)
document.addEventListener('DOMContentLoaded', () => {
...
ws.onmessage = (event) => {
    const data = JSON.parse(event.data);
    if (data.type === 'new-server-message') {
        const channelIdStr = String(data.message.channel_id);  // String'e dönüştür
        const senderIdStr = String(data.message.sender_id);
        const currentChannelStr = String(currentChannelId1);
        const currentUserStr = String(currentUserId1);
        
        if (channelIdStr === currentChannelStr && senderIdStr !== currentUserStr) {
            console.log('[DEBUG] Yeni server mesajı alındı:', data.message);
            const messageContainer = document.getElementById('message-container');
            let previousMessageData = null;
            if (messageContainer.lastElementChild) {
                const lastMessage = messageContainer.lastElementChild;
            ...
            const newMessageElement = createNewMessageElement(data.message, previousMessageData);
            ...
            console.log('[DEBUG] Mesaj filtrelendi: channel match?', channelIdStr === currentChannelStr, 
                       'sender not self?', senderIdStr !== currentUserStr);
        }
    }
    // Diğer handler'lar
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    serverList.addEventListener('contextmenu', (e) => {
        ...
        if (serverItem) {
            ...
            contextMenu.dataset.serverId = serverId;
    ...
    // Context menüsü aksiyonları
    contextMenu.addEventListener('click', (e) => {
        const actionItem = e.target.closest('.server-context-item');
        if (actionItem) {
            const action = actionItem.dataset.action;
            const serverId = contextMenu.dataset.serverId;

            if (action === 'mark-all-read' && serverId) {
                markAllServerMessagesRead(serverId);
            }
        }
        contextMenu.classList.add('hidden');
    });

    // Menüyü kapat (dışarı tıklama)
    document.addEventListener('click', (e) => {
        if (!contextMenu.contains(e.target) && !e.target.closest('.server-item')) {
            contextMenu.classList.add('hidden');
        }
    });

    // Tüm mesajları okundu olarak işaretleme
    function markAllServerMessagesRead(serverId) {
        fetch('mark_all_server_messages_read.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: `server_id=${serverId}`
        })
        ...
        .then(data => {
            if (data.success) {
                // Okunmamış bildirim rozetlerini kaldır
                const serverItem = document.querySelector(`.server-item[data-server-id="${serverId}"]`);
                if (serverItem) {
                    const badge = serverItem.querySelector('.unread-badge, .absolute.left-0.top-1\\/2');
                    if (badge) badge.remove();
                }
                showTempNotification('Tüm mesajlar okundu olarak işaretlendi.', 'success');
            } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const emojiOption = document.getElementById('emoji-button');
    const gifOption = document.getElementById('gif-button');
    const pollOption = document.getElementById('poll-button');
    ...
    // Tıklama dışında menüyü kapatma
    document.addEventListener('click', (e) => {
    ...
    emojiOption.addEventListener('click', () => {
        // Emoji seçici işlevselliği (örneğin, bir emoji kütüphanesi entegre edilebilir)
        console.log('Emoji seçici açıldı');
        moreMenu.classList.add('hidden');
        // Örnek: Emoji kütüphanesi entegrasyonu için
        // emojiPicker.showPicker(document.getElementById('message-input'));
    });

    // GIF seçeneği
    gifOption.addEventListener('click', () => {
        // GIF seçici işlevselliği (örneğin, GIPHY API entegrasyonu)
        console.log('GIF seçici açıldı');
        moreMenu.classList.add('hidden');
    });

    // Anket oluşturma seçeneği
    pollOption.addEventListener('click', () => {
        // Anket oluşturma işlevselliği
        console.log('Anket oluşturma paneli açıldı');
        moreMenu.classList.add('hidden');
    });
});
...
// Öneri listesi için referanslar
const messageInput = document.getElementById('message-input');
const mentionSuggestions = document.getElementById('mention-suggestions');
const suggestionList = document.getElementById('suggestion-list');

// Öneri listesini göster/gizle
function toggleSuggestions(show) {
    mentionSuggestions.classList.toggle('hidden', !show);
}
...
function renderSuggestions(query = '') {
    ...
    const filteredMembers = serverMembers.filter(member => {
        const username = (member.display_username || member.username).toLowerCase();
        return query ? username.includes(query.toLowerCase()) : true;
    });

    if (filteredMembers.length === 0) {
        suggestionList.innerHTML = '<li class="p-3 text-gray-500">Kullanıcı bulunamadı</li>';
        return;
    }
    ...
    filteredMembers.forEach(member => {
        ...
        const displayName = member.display_username || member.username;
        const avatarUrl = member.avatar_url || 'https://via.placeholder.com/24';
        const highlightedName = query
            ? displayName.replace(new RegExp(query, 'gi'), match => `<span class="highlight">${match}</span>`)
            : displayName;
...
function insertMention(username) {
    ...
    if (lastAtIndex !== -1) {
        ...
        messageInput.selectionStart = messageInput.selectionEnd = lastAtIndex + username.length + 2;
...
// Mesaj girişi olay dinleyicisi
messageInput.addEventListener('input', () => {
    const cursorPos = messageInput.selectionStart;
    const textBefore = messageInput.value.substring(0, cursorPos);
    const lastAtIndex = textBefore.lastIndexOf('@');

    if (lastAtIndex !== -1 && (cursorPos === lastAtIndex + 1 || !/\s/.test(textBefore.slice(lastAtIndex + 1)))) {
...
messageInput.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        toggleSuggestions(false);
    } else if (e.key === 'Tab' && !mentionSuggestions.classList.contains('hidden')) {
        e.preventDefault();
        const firstSuggestion = suggestionList.querySelector('li');
        if (firstSuggestion) {
            const username = firstSuggestion.dataset.username;
...
// Mesaj alanından çıkıldığında önerileri gizle
messageInput.addEventListener('blur', () => {
    setTimeout(() => toggleSuggestions(false), 200);
});

// Dışarı tıklama ile önerileri gizle
document.addEventListener('click', (e) => {
